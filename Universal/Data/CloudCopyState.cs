...
namespace Universal.Data {
    public class CloudCopyState {
        ...
        public const int BUFFER_LENGTH = 1 * sizeof(int) + 1 * sizeof(long);
        ...
        public static byte[ ] GetData (long Gold) {
        ...
        public async static Task<long> Update (long Gold, GoogleApiClient googleApiClient) {
            ISnapshot snapshot = await GetSnapshot(googleApiClient);

            snapshot.SnapshotContents.WriteBytes(CloudCopyState.GetData(Gold));
